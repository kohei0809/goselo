    ...
    for i in range(-2, 2):
        for j in range(-2, 2):
            ...
            if(yA+i >= 0) and (xA+j >= 0) and (yA+i < m) and (xA+j < n):
                if im_map_[yA+i][xA+j][0] == 255:
                    im_map_[yA+i][xA+j][0] = 0
                    im_map_[yA+i][xA+j][1] = 0
            if(yB+i >= 0) and (xB+j >= 0) and (yB+i < m) and (xB+j < n):
                if im_map_[yB+i][xB+j][0] == 255:
                    im_map_[yB+i][xB+j][1] = 0
                    im_map_[yB+i][xB+j][2] = 0
                    
                
            
                    
    cv2.imshow('map',im_map_)
    cv2.waitKey()
    cv2.destroyAllWindows()
...
if change_color:
    ...
    for i in range(-2, 2):
        for j in range(-2, 2):
            #print("i=" + str(i) + ", j=" + str(j))
            if(yA+i >= 0) and (xA+j >= 0) and (yA+i < m) and (xA+j < n):
                im_map_[yA+i][xA+j][0] = 255
                im_map_[yA+i][xA+j][1] = 255
            if(yB+i >= 0) and (xB+j >= 0) and (yB+i < m) and (xB+j < n):
                im_map_[yB+i][xB+j][1] = 255
                im_map_[yB+i][xB+j][2] = 255
##################
# expand the map #
##################
#白色の領域を収縮
...
#for i in range(20):
    ...
    #print(im3[i])
print(im3[0])
cv2.imshow('map',im3[0])
cv2.waitKey()
cv2.destroyAllWindows()

print(im3[1].shape)
cv2.imshow('map',im3[1])
cv2.waitKey()
cv2.destroyAllWindows()

print(im3[2].shape)
cv2.imshow('map',im3[2])
cv2.waitKey()
cv2.destroyAllWindows()
